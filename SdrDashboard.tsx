...
export default function SdrDashboard({ 
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // Manual company name (SDR-provided) overrides extraction so the
      // historical comparison and the saved record use the human-confirmed name.
      const manualCompany = companyNameInput.trim();
      if (manualCompany) {
        analyzedPayload.companyName = manualCompany;
      }
      ...
      const newOpp: SalesforceOpportunity = {
        ...
        companySource: manualCompany ? 'manual' : 'extracted',
    ...
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div id="ingestion-control-card" className="bg-[#0e1626] rounded-xl border border-slate-800 shadow-xl overflow-hidden">
        ...
        <div className="p-6 space-y-6 bg-[#0e1626]">
          ...
          {/* Company name (optional, SDR-provided) */}
          <div className="bg-slate-900/40 border border-slate-800 rounded-xl px-4 py-3">
            <div className="flex items-center justify-between mb-1.5">
              <label htmlFor="company-name-input" className="text-xs font-bold text-slate-200 tracking-wide uppercase font-sans">
                Company name <span className="text-slate-500 normal-case font-semibold">(optional)</span>
              </label>
              {companyNameInput && (
                <button
                  type="button"
            ...
            <input
              ...
              disabled={loading}
              placeholder="Type or pick the account, e.g. Northwind Logistics"
